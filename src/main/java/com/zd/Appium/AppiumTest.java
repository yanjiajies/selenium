...
public class AppiumTest {
    ...
    public void init() throws MalformedURLException {
        ...
        des.setCapability("udid","192.168.115.101:5555");//连接的物理设备的唯一设备标识(可以通过adb devices命令获取)
        des.setCapability("deviceName","S7");//使用手机类型或者模拟器类型
        //app安装后的包名（在启动app后可以通过'adb shell dumpsys window w |findstr \/ |findstr name='命令获取）
        des.setCapability("appPackage","com.sx.huayun");
        des.setCapability("appActivity","com.sx.huayun.activity.LoginActivity");//app测试人员常常要获取activity，进行相关测试(获取方法同上)
        ...
        des.setCapability("appWaitActivity","com.sx.huayun.activity.LoginActivity");//Value和appActivity配置一致
